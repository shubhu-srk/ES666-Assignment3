...
class PanaromaStitcher:
    def __init__(self):
        ...
        # FLANN-based matcher for better performance of the algorithm
        index_params = dict(algorithm=1, trees=5)
        search_params = dict(checks=50)  # Specifying the number of types traversing the tree
    ...
    def make_panaroma_for_images_in(self, path):
        ...
        for i in range(1, len(images)):
            ...
            if len(good_matches) < 6:
                logger.warning(f"Not enough good matches between image {i} and image {i-1}, Hence, skipping this pair")
                continue
            ...
            if H is None:
                logger.warning(f"Failed to compute homography for image {i} and image {i-1}. Hence,Skipping this pair.")
                continue
